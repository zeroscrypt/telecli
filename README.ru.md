🇷🇺 Русский | [🇬🇧 English](README.md)

# telecli

Терминальный клиент Telegram с vim-подобной модальностью ввода. Написан на Go, протокольное ядро — [TDLib](https://github.com/tdlib/td) через прямой cgo-биндинг.

- Три панели: папки → чаты → лента сообщений, переключение `Tab`/стрелками/цифрами `1`/`2`/`3`.
- Режимы `Normal`/`Insert`/`Command`, как в vim.
- Живые обновления входящих сообщений без перезахода в чат.
- Многострочный черновик, отправка файлов (`ctrl+f`).
- Отправка текста и файлов прямо из shell, без входа в TUI: `telecli send ...`.
- Настраиваемые горячие клавиши (`keybindings.toml`) и опции интерфейса (`settings.toml`).

## Установка

### Быстрее всего: однострочный инсталл-скрипт

Сам определяет платформу (macOS Apple Silicon или Linux x86_64), скачивает подходящий готовый
бинарник и ставит его как `telecli` в ваш `PATH` — TDLib ставить не нужно, файл выбирать самому
не нужно:

```sh
curl -fsSL https://raw.githubusercontent.com/zeroscrypt/telecli/public/install.sh | sh
```

Хотите сначала посмотреть, что делает скрипт, прежде чем пускать его в `sh`? Он короткий:
[`install.sh`](install.sh). Либо используйте ручные команды по платформам ниже — тот же
результат, один лишний шаг (выбрать файл под свою ОС самому).

Всего три способа получить `telecli`:

- **Скрипт выше** — быстрее всего, сам работает для обеих платформ ниже.
- **[Вариант A — macOS (Apple Silicon), готовый бинарник, вручную](#вариант-a--macos-apple-silicon-готовый-бинарник)** — тот же результат, что скрипт, явные команды.
- **[Вариант B — Linux (x86_64), готовый бинарник, вручную](#вариант-b--linux-x86_64-готовый-бинарник)** — то же самое, для Linux.
- **[Вариант C — сборка из исходников](#вариант-c--сборка-из-исходников)** — любая платформа; для
  тех, кто хочет проверить, что запускает, или у кого платформа не из двух выше.

### Вариант A — macOS (Apple Silicon), готовый бинарник

```sh
mkdir -p ~/.local/bin
curl -fsSL -o ~/.local/bin/telecli https://github.com/zeroscrypt/telecli/releases/latest/download/telecli-darwin-arm64
chmod 755 ~/.local/bin/telecli
```

Если `~/.local/bin` ещё не в вашем `PATH` — добавьте эту строку в `~/.zshrc` (или `~/.bashrc`) и
перезапустите терминал:

```sh
export PATH="$HOME/.local/bin:$PATH"
```

Дальше просто запускайте `telecli` из любой папки. (Скачано через `curl`, не через браузер — macOS
Gatekeeper файл не карантинит, лишнего шага «разрешить в настройках безопасности» не будет.)

Чтобы переустановить/обновить позже — выполните те же три строки ещё раз, `curl -o` перезапишет
старый файл.

### Вариант B — Linux (x86_64), готовый бинарник

```sh
mkdir -p ~/.local/bin
curl -fsSL -o ~/.local/bin/telecli https://github.com/zeroscrypt/telecli/releases/latest/download/telecli-linux-amd64
chmod 755 ~/.local/bin/telecli
```

Если `~/.local/bin` ещё не в вашем `PATH` — добавьте эту строку в `~/.bashrc` (или аналог для вашей
оболочки) и перезапустите терминал:

```sh
export PATH="$HOME/.local/bin:$PATH"
```

Дальше просто запускайте `telecli` из любой папки. Обновление — теми же тремя строками ещё раз.

*(Оба варианта выше — статическая линковка, TDLib ставить отдельно не нужно, других зависимостей во
время работы нет.)*

### Вариант C — сборка из исходников

Для платформ без готового бинарника, или если хотите собрать сами, а не запускать чужой бинарник
без проверки.

#### Зависимости

- Go 1.25 или новее.
- Собранная из исходников TDLib (см. ниже — **не через пакетный менеджер**, это важно).
- Данные приложения Telegram (`api_id`/`api_hash`) — бесплатно регистрируются на [my.telegram.org](https://my.telegram.org).

#### 1. Собрать TDLib

Пакетные версии TDLib (например, `brew install tdlib` на macOS) в лучшем случае устарели на несколько
лет и Telegram отклоняет вход с такой версией ошибкой `UPDATE_APP_TO_LOGIN`. Собирайте из актуального
`master`, не из старого тега.

**macOS:**

```sh
brew install cmake gperf openssl@3 zlib

git clone --depth 1 https://github.com/tdlib/td.git
cd td && mkdir build && cd build
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="$HOME/.local/tdlib" \
      -DOPENSSL_ROOT_DIR=/opt/homebrew/opt/openssl@3 -DZLIB_ROOT=/opt/homebrew/opt/zlib ..
cmake --build . --target tdjson --target tdjson_static -- -j8
cmake --install . --prefix "$HOME/.local/tdlib"
```

После установки собранная из `master` `libtdjson` использует `@rpath`-относительное имя — без явного
`rpath` бинарник упадёт с `Library not loaded: @rpath/libtdjson....dylib`. Один раз после сборки
добавьте флаг `-Wl,-rpath,"${prefix}/lib"` в строку `Libs:` файла
`~/.local/tdlib/lib/pkgconfig/tdjson.pc` (после `-L...`, перед `-ltdjson`; `cmake --install` перезатрёт
этот файл при пересборке TDLib — патч нужно будет повторить).

**Linux (Ubuntu/Debian, не проверено физически, задокументировано по официальным источникам):**

```sh
sudo apt-get update && sudo apt-get install -y git cmake g++ openssl libssl-dev zlib1g-dev gperf

git clone --depth 1 https://github.com/tdlib/td.git
cd td && mkdir build && cd build
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$HOME/.local/tdlib ..
cmake --build . --target tdjson --target tdjson_static -j$(nproc)
cmake --install . --prefix $HOME/.local/tdlib
```

Нужно ≥4 ГБ RAM на этапе компиляции. `.pc`-файл, вероятно, тоже нуждается в `-rpath`-патче — сверьте
по факту, если получите ошибку загрузки библиотеки при запуске.

#### 2. Собрать telecli

```sh
git clone https://github.com/zeroscrypt/telecli.git
cd telecli
PKG_CONFIG_PATH="$HOME/.local/tdlib/lib/pkgconfig" go build -o telecli ./cmd/telecli
```

Если на машине также установлен старый пакетный TDLib — убедитесь, что
`$HOME/.local/tdlib/lib/pkgconfig` идёт первым в `PKG_CONFIG_PATH`, иначе можно случайно собраться со
старой версией.

### Получить данные приложения Telegram

Нужно независимо от того, какой вариант (A/B/C) вы использовали выше.

Зарегистрируйте приложение на [my.telegram.org](https://my.telegram.org) → «API development tools» →
получите `api_id` и `api_hash`. Передайте их один раз через переменные окружения при первом запуске —
дальше они сохранятся в системном хранилище учётных данных (Keychain на macOS, Secret Service на
Linux), запрашивать заново не придётся:

```sh
TELECLI_API_ID=12345678 TELECLI_API_HASH=abcdef0123456789abcdef0123456789 ./telecli
```

Если системное хранилище недоступно — данные сохраняются в
`<config dir>/telecli/config.toml` (macOS: `~/Library/Application Support/telecli/config.toml`,
Linux: `~/.config/telecli/config.toml`).

При первом запуске также потребуется пройти стандартную авторизацию Telegram (номер телефона, код
подтверждения, при необходимости — облачный пароль) — запрашивается интерактивно в терминале.

## Использование

```sh
telecli                                        # запустить TUI
telecli send @username -m "привет"             # отправить текст без входа в TUI
telecli send @username -f file.png -m "подпись"
telecli send 123456789 -m "по числовому chat_id"
telecli send -m "для канала" -- -1001234567890 # отрицательный chat_id — после "--"
```

## Горячие клавиши (Normal-режим, по умолчанию)

| Клавиша | Действие |
|---|---|
| `↑`/`k`, `↓`/`j` | навигация по списку в активной панели |
| `Tab` | переключить фокус (циклически) |
| `←`/`→` | фокус на соседнюю панель (без зацикливания) |
| `1` / `2` / `3` | прямой переход: папки / чаты / сообщения |
| `Enter` | выбрать/открыть |
| `Esc` | назад |
| `i` | войти в режим ввода (нужен выбранный чат) |
| `:` | командная строка (`:q`/`:quit`, `:help`, `:update`/`:update install`, `:theme <имя>`) |
| `/` | поиск чатов/каналов/контактов |
| `h` | окно справки — полный список хоткеев (то же самое, что `:help`) |
| `t` | экран «о программе» — логотип TELECLi, версия, ссылка, автор |
| `d` | покинуть/удалить чат под курсором (с подтверждением) |
| `ctrl+f` | отправить файл (ввод пути) |
| `p` | воспроизвести голосовое / показать фото inline в iTerm2 |
| `q`, `ctrl+c` | выход |

В режиме ввода: `Enter` — отправить, `ctrl+j` — перенос строки, `Esc` — отменить и выйти.

Inline-фото скачивается только по нажатию `p` и показывается только при точном значении
`TERM_PROGRAM=iTerm.app`; в остальных терминалах остаётся плейсхолдер `[фото]`.

*(Почему `ctrl+j`, а не `Shift+Enter`: в большинстве терминалов `Shift+Enter` посылает ровно те же
байты, что и обычный `Enter` — отличить их можно только через Kitty keyboard protocol, а
[bubbletea](https://github.com/charmbracelet/bubbletea) (TUI-библиотека, на которой построено
приложение) его пока не поддерживает даже в последнем релизе. `ctrl+j` работает одинаково везде.)*

Все клавиши переопределяются в `<config dir>/telecli/keybindings.toml` (создаётся вручную, формат —
`toml`, поля — списки строк на действие, отсутствующие поля используют значения по умолчанию).

## Настройки интерфейса

`<config dir>/telecli/settings.toml`:

```toml
align_own_right = true    # прижимать свои сообщения к правому краю (по умолчанию включено)
theme = "neon"              # neon (по умолчанию) / yellow / blue / terracotta
```

Тему можно переключить и на лету — `:theme <имя>` в командной строке — но это меняет только
текущий запуск, в `settings.toml` не записывается; чтобы тема осталась по умолчанию при следующем
запуске, впишите её в файл вручную.

## Проверка обновлений

При запуске `telecli` тихо проверяет в фоне, нет ли более новой версии (GitHub Releases этого
репозитория) — при найденном обновлении справа в нижней строке появляется `vX.Y.Z → vX.Y.Z+1
(:update)`. В командной строке (`:`):

- `:update` — проверить вручную, результат показывается явно (доступна новее / уже последняя / ошибка).
- `:update install` — скачать и установить обновление на месте (заменяет работающий бинарник на
  диске; чтобы реально перейти на новую версию, `telecli` нужно перезапустить).

Обновиться можно и вручную в любой момент — скачать новый релиз со страницы
[Releases](https://github.com/zeroscrypt/telecli/releases), заново прогнать [инсталл-скрипт](#быстрее-всего-однострочный-инсталл-скрипт)/команды варианта A/B выше (они перезапишут старый
бинарник), или пересобрать из исходников (вариант C).

## Релизы

Версии присваиваются автоматически инструментом [Release Please](https://github.com/googleapis/release-please)
по [Conventional Commits](https://www.conventionalcommits.org/) в истории коммитов этой ветки:

| Префикс коммита | Результат |
|---|---|
| `fix: ...` | patch (0.1.0 → 0.1.1) |
| `feat: ...` | minor (0.1.0 → 0.2.0) |
| `feat!: ...` / `fix!: ...` (или футер `BREAKING CHANGE:`) | major (0.1.0 → 1.0.0) |

При каждом пуше сюда Release Please обновляет открытый Release PR с накопленным `CHANGELOG.md` и
следующей версией; при мерже этого PR создаётся git-тег и GitHub Release. Публикация релиза
автоматически запускает второй workflow, который собирает бинарники (вариант A/B выше) и
прикрепляет их к релизу — без ручного шага.

## Лицензия

MIT — см. [LICENSE](LICENSE).

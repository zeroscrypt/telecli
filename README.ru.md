...
Три способа получить `telecli`, выберите один:

- **[Вариант A — macOS (Apple Silicon), готовый бинарник](#вариант-a--macos-apple-silicon-готовый-бинарник)** — быстрее всего, TDLib ставить не нужно.
- **[Вариант B — Linux (x86_64), готовый бинарник](#вариант-b--linux-x86_64-готовый-бинарник)** — то же самое, для Linux.
...
время работы нет. Хотите одну команду, которая сама определит платформу? См.
[`install.sh`](install.sh) — `curl -fsSL https://raw.githubusercontent.com/zeroscrypt/telecli/public/install.sh | sh` делает то же самое, что вариант A/B, просто не нужно выбирать файл самому. Прежде
чем пускать в `sh`, можно сначала посмотреть, что в скрипте — он короткий.)*
...
| `t` | окно «о программе» — полный список хоткеев |
...
[Releases](https://github.com/zeroscrypt/telecli/releases) (команды варианта A/B выше перезапишут
старый бинарник) или пересборкой из исходников (вариант C).
